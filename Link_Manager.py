...
class Link_Manager():
    def __init__(self):
        ...
        try:
            self.df10Q = pd.read_csv(
                self.base_link + 'Data/Dataframes/10_Q.csv')
            self.df10K = pd.read_csv(
                self.base_link + 'Data/Dataframes/10_K.csv')
            self.df8K = pd.read_csv(self.base_link + 'Data/Dataframes/8_K.csv')
        except:
            pass
    ...
    def _parse_idx(self):
        ...
        company = []
        flag = True
        with open(self.base_link+'Data/Temp/index.idx', encoding='Latin_1') as f:
            for line in f:
                if flag:
                    try:
                        if line.split()[0] == '1-A':
    ...
    def _get_idx(self, year, Qtr):
        ...
        with open(self.base_link+'Data/Temp/index.idx', 'wb') as f:
            f.write(r.content)

    def fetch_old_index(self, years=None):
        """Function to get all old index. Run this once every Quarter"""
        try:
            os.remove(self.base_link + 'Data/Dataframes/10_K_old.csv')
            os.remove(self.base_link + 'Data/Dataframes/8_K_old.csv')
            os.remove(self.base_link + 'Data/Dataframes/10_Q_old.csv')
        except:
            pass
        if years == None:
        ...
        for year in years:
            for i in range(1, 5):
                ...
                try:  # Fail silently, form Not available if failed:/
                    groups.get_group(
                        '10-K').to_csv(self.base_link + 'Data/Dataframes/10_K_old.csv', mode=mode, index=False)
                except:
                    pass
                try:
                    groups.get_group(
                        '8-K').to_csv(self.base_link + 'Data/Dataframes/8_K_old.csv', mode=mode, index=False)
                except:
                    pass
                try:
                    groups.get_group(
                        '10-Q').to_csv(self.base_link + 'Data/Dataframes/10_Q_old.csv', mode=mode, index=False)
                except:
                    pass
    ...
    def update(self):
        ...
        try:
            os.remove(self.base_link + 'Data/Dataframes/10_K.csv')
            os.remove(self.base_link + 'Data/Dataframes/8_K.csv')
            os.remove(self.base_link + 'Data/Dataframes/10_Q.csv')
        except:
            pass
        ...
        df_old = pd.read_csv(self.base_link + 'Data/Dataframes/10_K_old.csv')
        self.df10K = pd.concat([df_old, groups.get_group('10-K')],
                               axis=0).reset_index(drop=True)
        df_old = pd.read_csv(self.base_link + 'Data/Dataframes/10_Q_old.csv')
        self.df10Q = pd.concat([df_old, groups.get_group('10-Q')],
                               axis=0).reset_index(drop=True)
        df_old = pd.read_csv(self.base_link + 'Data/Dataframes/8_K_old.csv')
        self.df8K = pd.concat([df_old, groups.get_group('8-K')],
                              axis=0).reset_index(drop=True)
        self.df10Q.to_csv(
            self.base_link + 'Data/Dataframes/10_Q.csv', index=False)
        self.df10K.to_csv(
            self.base_link + 'Data/Dataframes/10_K.csv', index=False)
        self.df8K.to_csv(self.base_link +
                         'Data/Dataframes/8_K.csv', index=False)
    ...
    def load(self):
        self.df10Q = pd.read_csv(self.base_link + 'Data/Dataframes/10_Q.csv')
        self.df10K = pd.read_csv(self.base_link + 'Data/Dataframes/10_K.csv')
        self.df8K = pd.read_csv(self.base_link + 'Data/Dataframes/8_K.csv')
